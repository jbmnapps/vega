...
function App() {
  ...
  const [modal, setModal] = React.useState(null); // 'foder-log' | 'vaegt-log' | 'obs-new' | 'foder-new-product' | 'profile' | 'add-post' | 'produkter' | 'vaegt-detail' | 'vaegt-malinger'
  const [modalCtx, setModalCtx] = React.useState(null); // extra context: { editPostId } | { foderLogDate }
  ...
  const onOpenLogger = (kind, ctx) => { setModalCtx(ctx || null); setModal(kind); };
  const onCloseModal = () => { setModal(null); setModalCtx(null); };

  const renderScreen = () => {
    if (tab === 'idag') return <IdagScreen app={app} onOpenLogger={onOpenLogger} onOpenProfile={() => setModal('profile')} onAddPost={() => onOpenLogger('add-post')} onEditPost={(id) => onOpenLogger('add-post', { editPostId: id })} />;
  ...
  const renderModal = () => {
    ...
    let content;
    if (modal === 'foder-log') content = <FoderLogScreen app={app} initialDate={modalCtx?.foderLogDate} {...common} />;
    ...
    else if (modal === 'add-post') content = <AddPostScreen app={app} editPostId={modalCtx?.editPostId} {...common} />;
    else if (modal === 'produkter') content = <ProdukterScreen app={app} {...common} />;

    const pushFromRight = modal === 'produkter';
    const modalKey = modal + (modalCtx ? JSON.stringify(modalCtx) : '');
    return (
      <div key={modalKey} style={{
        position: 'absolute', inset: 0, zIndex: 60,
        background: TOKENS.bg,
        animation: `${pushFromRight ? 'slideInRight' : 'slideUp'} 280ms cubic-bezier(0.2, 0.8, 0.2, 1)`,
